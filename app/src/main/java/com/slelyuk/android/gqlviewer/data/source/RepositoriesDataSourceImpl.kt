...
class RepositoriesDataSourceImpl(
     ...
     */
    var cachedRepos: LinkedHashMap<String, RepositoryItem> = LinkedHashMap()
    ...
    private suspend fun getTasksFromRemoteDataSource(): Result<List<RepositoryItem>> {
    ...
    private fun refreshCache(repos: List<RepositoryItem>) {
        cachedRepos.clear()
        repos.forEach {
            cache(it)
        }
    ...
    private fun cache(repo: RepositoryItem): RepositoryItem {
        cachedRepos[repo.nameWithOwner()] = repo
        return repo
    }
